// src/pages/admin/TemplateViewPage.tsx
...
type TemplateType = 'lesson' | 'community' | string
type SectionType = 'text' | 'image' | 'video' | 'quiz' | 'discussion' | 'events' | 'resources' | string

interface QuizConfig { minQuestions?: number; maxQuestions?: number; passingScore?: number }
interface DiscussionConfig { requireApproval?: boolean }
...
export default function TemplateViewPage(_props: TemplateViewPageProps) {
  const { id } = useParams<{ id: string }>()
  const navigate = useNavigate()
  const { user, userRoles } = (useAuth() as any) || {}
    ...
    const fetchTemplate = async () => {
      try {
        ...
        if (templateDoc.exists()) {
          setTemplate({ id: templateDoc.id, ...(templateDoc.data() as TemplateDoc) })
        } else {
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-6">
            ...
            <div className="flex items-center mt-1 space-x-2">
              <span
                className={`inline-block px-2 py-1 text-xs font-semibold rounded-full ${
                  template.type === 'lesson' ? 'bg-green-100 text-green-800' : 'bg-purple-100 text-purple-800'
                }`}
              >
          ...
          <div className="space-x-2">
            <button onClick={() => navigate('/admin/templates')} className="px-3 py-1 border border-gray-300 rounded text-gray-700 hover:bg-gray-50">
              Back to Templates
            </button>

            {isSuperAdmin && (
              <button onClick={() => navigate(`/admin/templates/${id}/edit`)} className="px-3 py-1 bg-blue-600 text-white rounded hover:bg-blue-700">
                Edit Template
              </button>
            )}
          </div>
        ...
        <div className="border-b border-gray-200 mb-6">
          <nav className="flex space-x-8">
            <button
              ...
              className={`pb-4 px-1 ${activeTab === 'preview' ? 'border-b-2 border-blue-500 text-blue-600 font-medium' : 'text-gray-500 hover:text-gray-700 hover:border-gray-300'}`}
            >
              Preview
            </button>
            <button
              onClick={() => setActiveTab('structure')}
              className={`pb-4 px-1 ${activeTab === 'structure' ? 'border-b-2 border-blue-500 text-blue-600 font-medium' : 'text-gray-500 hover:text-gray-700 hover:border-gray-300'}`}
            >
              Template Structure
            </button>
          </nav>
        ...
        {activeTab === 'preview' ? (
          <div>
              ...
              <div className="inline-flex rounded-md shadow-sm" role="group">
                <button
                  ...
                  className={`px-4 py-2 text-sm font-medium rounded-l-lg ${
                    viewMode === 'desktop' ? 'bg-blue-600 text-white' : 'bg-white text-gray-700 border border-gray-300 hover:bg-gray-50'
                  }`}
                >
                ...
                <button
                  ...
                  className={`px-4 py-2 text-sm font-medium rounded-r-lg ${
                    viewMode === 'mobile' ? 'bg-blue-600 text-white' : 'bg-white text-gray-700 border border-gray-300 hover:bg-gray-50'
                  }`}
                >
                  <span className="flex items-center">
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div className="flex justify-center pb-12">
              <div className={`${viewMode === 'mobile' ? 'w-full max-w-xs' : 'w-full max-w-4xl'} transition-all duration-300`}>
                <div className={`bg-white border rounded-lg shadow-lg overflow-hidden ${viewMode === 'mobile' ? 'mx-auto' : ''}`}>
                  {viewMode === 'mobile' && (
                  ...
                  <div className={`${viewMode === 'mobile' ? 'p-3' : 'p-6'}`}>
                    <h2 className="text-lg font-bold mb-4 text-center border-b pb-2">{template.name}</h2>

                    {/* Render template sections */}
                    {template.sections && template.sections.length > 0 ? (
                      <div className="space-y-6">
                        {template.sections.map((section) => (
                          <TemplateSectionPreview key={section.id} section={section} viewMode={viewMode} />
                        ))}
                      </div>
                    ) : (
                      <div className="text-center py-8 text-gray-500 italic">This template has no sections defined yet.</div>
                    )}
                  </div>
                </div>
              </div>
                    ...
                    <div key={section.id} className="border border-gray-200 rounded-md p-4">
                      ...
                      {section.type === 'quiz' && section.quizConfig && (
                        <div className="mt-2 pt-2 border-t border-gray-100">
                          <p className="text-sm text-gray-600">
                            ...
                            {section.quizConfig.passingScore && ` (${section.quizConfig.passingScore}% passing score)`}
                          </p>
                        </div>
                      )}
                      ...
                      {section.type === 'discussion' && section.discussionConfig && (
                        <div className="mt-2 pt-2 border-t border-gray-100">
                          <p className="text-sm text-gray-600">
                            Discussion board
                            {section.discussionConfig.requireApproval ? ' (posts require approval)' : ' (open posting)'}
                          </p>
                        </div>
                      )}
                    </div>
...
// Preview component
function TemplateSectionPreview({ section, viewMode }: { section: TemplateSection; viewMode: 'desktop' | 'mobile' }) {
  switch (section.type) {
    case 'text':
      return (
        ...
        <div className="border border-gray-200 rounded-md p-4">
          ...
          <div className="bg-blue-50 rounded-md p-4">
            ...
            <div className="mt-4 text-sm text-gray-500">
              {section.quizConfig ? (
                <p>
                  {section.quizConfig.minQuestions || 1} to {section.quizConfig.maxQuestions || 5} questions with {section.quizConfig.passingScore || 70}% passing score
                </p>
              ) : (
    ...
    default:
      return (
        <div className="border border-gray-200 rounded-md p-4">
          <h3 className="font-medium text-gray-800 mb-2">{section.title}</h3>
          <p className="text-gray-500 italic">{section.placeholder || `${section.type} content will appear here`}</p>
        </div>
      )
  }
}
