...
export default function ContentManagement(): JSX.Element {
  ...
  const getTypeIcon = (type: ContentKind) => {
    switch (type) {
      case 'lesson':
  ...
  const handleApproveContent = (contentId: string) => {
    setContent((prev) =>
      prev.map((item) => (item.id === contentId ? { ...item, status: 'published', publishedDate: new Date().toISOString().split('T')[0] } : item))
    )
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Manage lessons, challenges, and educational content for all brands</p>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-6">
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">{summaryMetrics.publishedContent} published</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">From additional content</p>
          </CardContent>
        </Card>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Manage all lessons and challenges across your brand partnerships</CardDescription>
        </CardHeader>
        <CardContent>
                ...
                {filteredContent.map((item) => (
                  <TableRow key={item.id}>
                    ...
                    <TableCell>
                      ...
                      <div className="text-sm text-muted-foreground">{item.pricing.isAdditional ? 'Additional Content' : 'Base Plan'}</div>
                    </TableCell>
                    <TableCell>{getStatusBadge(item.status)}</TableCell>
                    <TableCell>
                      <div className="space-y-1">
                        <div className="text-sm">{item.metrics.totalViews.toLocaleString()} views</div>
                        <div className="text-sm text-muted-foreground">{item.metrics.completionRate.toFixed(1)}% completion</div>
                      </div>
                    </TableCell>
                    <TableCell>
                      <div className="space-y-1">
                        <div className="font-medium text-green-600">{formatCurrency(item.metrics.totalRevenue)}</div>
                        <div className="text-sm text-muted-foreground">
                    ...
                    <TableCell>
                      <div className="text-sm">{formatDate(item.lastModified)}</div>
                    </TableCell>
                          ...
                          {canAccess(['manage_content', 'approve_verifications']) && (
                            <>
                              ...
                              {item.status === 'pending_approval' && (
                                <>
                                  <DropdownMenuItem onClick={() => handleApproveContent(item.id)} className="text-green-600">
                                    <CheckCircle className="mr-2 h-4 w-4" />
                                    Approve
                                  </DropdownMenuItem>
                                  <DropdownMenuItem onClick={() => handleRejectContent(item.id)} className="text-red-600">
                              ...
                              <DropdownMenuItem onClick={() => handleContentAction('delete', item.id)} className="text-red-600">
      ...
      <Dialog open={detailsDialogOpen} onOpenChange={setDetailsDialogOpen}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>Detailed content information and performance metrics</DialogDescription>
          </DialogHeader>
            ...
            <Tabs defaultValue="overview" className="w-full">
              ...
              <TabsContent value="metrics" className="space-y-4">
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="text-3xl font-bold text-green-600 mb-2">{formatCurrency(selectedContent.metrics.totalRevenue)}</div>
                    <p className="text-sm text-muted-foreground">Total revenue attributed to this content piece</p>
                  </CardContent>
                </Card>
              </TabsContent>
                    ...
                    {selectedContent.pricing.isAdditional && (
                      <>
                        ...
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground capitalize">{selectedContent.pricing.billingCycle}</p>
                        </div>
                      </>
                    )}
      ...
      <Dialog open={uploadDialogOpen} onOpenChange={setUploadDialogOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>Add a new lesson or challenge to the content library</DialogDescription>
          </DialogHeader>
